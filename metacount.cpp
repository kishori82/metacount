...
int main(int argc, char **argv ){
  ...
  */
  ...
  RUN_STATS stats;
  RUN_STATS _stats;
  for (vector<string>::iterator it = options.read_map_files.begin(); 
       it != options.read_map_files.end(); it++)  {
    ...
    _stats = detect_multireads_blastoutput(*it, options.reads_map_file_format, 
                                           all_reads, multireads, options.show_status);
...
//        std::cout << "Number of mappable reads " << all_reads.size() << std::endl;
  ...
  for(map<string, CONTIG>::iterator it = contigs_dictionary.begin(); 
      it != contigs_dictionary.end(); it++) {
...
//    std::cout << "done\n";
  ...
  float total_distinct_reads = static_cast<double>(stats.num_distinct_reads_mapped) +
                               static_cast<double>(stats.num_distinct_reads_unmapped);
...
*/
  ...
  if (print_stats_file) {
    ...
    *output << buf  << std::endl;
    ...
    sprintf(buf,"Total Genone Length             : XXXX ");
